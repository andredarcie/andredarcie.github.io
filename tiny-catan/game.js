...
function renderActions(){
  ...
  const btn=(label,action,ico,secondary=false,disabled=false,wide=false)=>`<button class="action-btn ${secondary?'secondary':''} ${wide?'wide':''}" data-action="${action}" ${disabled?'disabled':''}>${icon(ico)} ${label}</button>`;
  if(G.winner!==null){kicker='FIM DA PARTIDA';title=G.winner===0?'Você conquistou Catan!':`${NAMES[G.winner]} venceu`;help='A primeira pessoa a chegar a 10 pontos vence.';buttons=btn('Jogar novamente','restart','rotate-ccw',false,false,true)}
    ...
    else if(G.phase==='main'){kicker='NEGOCIE · CONSTRUA';title='Sua vez';help='Faça suas ações em qualquer ordem e encerre quando quiser.';buttons=btn('Estrada','road','route',false,!canPay(p,COST.road)||p.roads>=15)+btn('Assentamento','settlement','house',false,!canPay(p,COST.settlement)||p.settlements>=5)+btn('Cidade','city','castle',false,!canPay(p,COST.city)||p.cities>=4||!p.settlements)+btn('Desenvolvimento','buyDev','badge-plus',false,!canPay(p,COST.dev)||!G.devDeck.length)+btn('Comerciar','trade','handshake',true)+btn('Jogar carta','dev','sparkles',true,!playableDevs(p).length)+btn('Encerrar turno','end','arrow-right',false,false,true)}
...
function endTurn(){G.players[G.active].dev.forEach(d=>d.new=false);G.active=(G.active+1)%3;if(G.active===G.startingPlayer)G.turn++;G.rolled=false;G.playedDev=false;G.boughtThisTurn=[];G.aiTradeWindowDone=false;G.phase='preRoll';$('#dice').classList.add('hidden');if(checkWin(G.active)){render();return}render();if(G.active!==0)setTimeout(aiTurn,450)}
...
function aiMain(pid){if(!G.aiTradeWindowDone){startAITradeWindow(pid);return}const p=G.players[pid];setTimeout(()=>{
    ...
    if(checkWin(pid)){render();return}const playable=p.dev.filter(d=>!d.new&&d.type!=='vp');if(!G.playedDev&&playable.length&&Math.random()<.35){playDev(playable[0].type,pid);return}render();setTimeout(endTurn,520)
  },650)}
