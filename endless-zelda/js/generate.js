// Endless Zelda — procedural world generator.
...
const generateWorld = (() => {
  ...
  function genOverworld(run) {
    ...
    // The sword cave ALWAYS sits on the spawn screen (classic "take this" opener),
    // so the hero begins every run right beside it.
    specials[start[0] + ',' + start[1]] = { tile: 'C', meta: { cave: 'sword' } };
    assign('C', { cave: 'shop' });
    assign('C', { cave: 'heart' });
    assign('C', { cave: 'hint' });
    assign('D', { dungeon: 1 });
