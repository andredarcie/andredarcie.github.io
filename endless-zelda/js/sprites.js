// Endless Zelda — pixel sprites (char grids) + tile renderer
'use strict';

// The Legend of Zelda ran on the NES 2C02 PPU. Every colour below is an exact
// entry of that 64-colour master palette (its $xx index is noted), so the whole
// game stays inside what the real hardware could actually output — no custom mixes.
const PAL = {
  G: '#00A800', // $1A link tunic green   (was #80D010 — a lime the NES can't make)
  S: '#FC9838', // $27 skin
  B: '#503000', // $08 brown / boots
  D: '#000000', // $0F black (outline / eyes)  (was #181818)
  W: '#FCFCFC', // $30 white
  R: '#D82800', // $16 red
  r: '#FC7460', // $26 light red
  O: '#E45C10', // $17 orange
  U: '#0058F8', // $12 blue
  u: '#3CBCFC', // $21 light blue
  Y: '#FCB040', // $28 gold
  y: '#FCE4A0', // $38 pale sand
  g: '#00A800', // $1A bright green
  d: '#005800', // $0B dark green
  K: '#BCBCBC', // $10 light gray
  k: '#787878', // $2D gray               (was #404040 — no such grey on the NES)
  P: '#6844FC', // $13 purple
};
...
const TILE = 16;
// All tile colours are NES 2C02 master-palette entries (index in comment).
const OW_COLORS = {
  ground: '#FCD8A8', groundDot: '#FC9838',   // $37 tan, $27 speckle
  sand: '#FCE4A0',                            // $38
  rock: '#E45C10', rockDark: '#881400',       // $17 / $07  (was #C84C0C/#7C2C00)
  tree: '#00A800', treeDark: '#005800',       // $1A / $0B
  water: '#0058F8', waterLight: '#3CBCFC',    // $12 / $21  (was #2038EC)
  bridge: '#AC7C00',                          // $18        (was #B87850)
};
...
function drawTile(ctx, ch, px, py, frame, env) {
  ...
  switch (ch) {
    ...
    case 's': // sand
      ...
      ctx.fillStyle = '#FC9838'; ctx.fillRect(px + 5, py + 7, 2, 2); ctx.fillRect(px + 12, py + 3, 2, 2);
      break;
    ...
    case 'w': // dungeon water
      ctx.fillStyle = '#0000BC'; ctx.fillRect(px, py, TILE, TILE);
      ctx.fillStyle = '#0058F8'; ctx.fillRect(px + ((frame >> 4) % 2 ? 2 : 8), py + 6, 5, 1);
      break;
...
// ---- tiny text ----
// Text is no longer painted on the canvas — it is drawn as crisp HTML on the
// overlay (see text.js). Kept the same signature so every call site works.
function drawText(ctx, str, x, y, color = '#FCFCFC') {
  TextLayer.text(str, x, y, color);
}
