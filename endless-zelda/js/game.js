// Endless Zelda — main engine: states, rooms, combat, HUD
'use strict';
...
window.addEventListener('keydown', e => {
  ...
  if (e.code === 'KeyV') { document.body.classList.toggle('crt'); return; } // CRT filter on/off
...
const G = {
  state: 'title',
  run: 1,                  // which endless run we are on (survives resetAll)
  genT: 0,                 // frames on the "generating your zelda" screen
  t: 0,
  ...
  visited: new Set(),      // overworld screens walked this run (minimap trail)
...
// ---------------- persistence (run counter, stats, meta-progression) ----------------
// Everything the player accumulates across sessions lives in one JSON blob so
// records, unlocks and the run counter all survive reloads.
const SAVE_KEY = 'endlessZelda.save';
function defaultSave() {
  return {
    run: 1,
    stats: { cleared: 0, deaths: 0, bosses: 0, secrets: 0, bestTime: 0, furthest: 1 },
    meta: { points: 0, unlocks: {} },   // unlocks: { heart, bombs, bow, ring, tunic }
  };
}
function loadSave() {
  try {
    const raw = localStorage.getItem(SAVE_KEY) || localStorage.getItem('infinityZelda.save');   // migrate old name
    if (raw) {
      const s = JSON.parse(raw), d = defaultSave();
      d.run = s.run || 1;
      Object.assign(d.stats, s.stats || {});
      Object.assign(d.meta, s.meta || {});
      d.meta.unlocks = Object.assign({}, (s.meta && s.meta.unlocks) || {});
      return d;
    }
    const legacy = parseInt(localStorage.getItem('infinityZelda.run'), 10);   // migrate old key
    const d = defaultSave();
    if (legacy >= 1) { d.run = legacy; d.stats.furthest = legacy; }
    return d;
  } catch (e) { return defaultSave(); }
}
const SAVE = loadSave();
function saveGame() { try { localStorage.setItem(SAVE_KEY, JSON.stringify(SAVE)); } catch (e) { /* no storage */ } }
function addPoints(n) { SAVE.meta.points += n; saveGame(); }
G.run = SAVE.run;

// ---------------- run mutators (risk/variety cards) ----------------
const MUTATORS = [
  { id: 'swarm',  name: 'SWARM',    desc: 'MORE ENEMIES' },
  { id: 'haste',  name: 'HASTE',    desc: 'ENEMIES ARE FASTER' },
  { id: 'frenzy', name: 'FRENZY',   desc: 'ENEMIES SHOOT MORE' },
  { id: 'glass',  name: 'GLASS',    desc: 'START AT HALF LIFE' },
  { id: 'dark',   name: 'DARKNESS', desc: 'THE LAND IS DARK' },
  { id: 'bounty', name: 'BOUNTY',   desc: 'BETTER DROPS' },
];
G.mods = [];
G.hasMod = id => G.mods.indexOf(id) >= 0;
function rollMutators() {
  const n = Math.min(3, 1 + Math.floor((G.run - 1) / 3));   // 1 early, up to 3 on deep runs
  const bag = MUTATORS.slice();
  G.mods = [];
  for (let i = 0; i < n && bag.length; i++) G.mods.push(bag.splice(Math.floor(Math.random() * bag.length), 1)[0].id);
}
function modName(id) { const m = MUTATORS.find(x => x.id === id); return m ? m.name : id; }

// ---------------- juice + menu state (not persisted) ----------------
G.shake = 0; G.hitStop = 0; G.combo = 0; G.comboT = 0; G.runT = 0; G.menu = 0; G.menuCd = 0;
...
function clearVolatiles() {
  // clear enemies too: a boss room never calls spawnEnemies, so without this
  // any survivors from the previous room would carry over into the boss fight
  G.enemies = [];
...
function loadOverworldScreen() {
  ...
  G.visited.add(G.screen.x + ',' + G.screen.y);   // minimap trail for this run
  renderBase();
...
// A cave/dungeon mouth is a solid tile. When it was entered from above, the
// stored return spot can land the hero straddling that solid tile and trap them.
// Nudge to the nearest free spot so we can never spawn stuck. (needs G.state
// already out of 'cave' so G.isSolid reports walls normally)
function unstickPlayer() {
  const p = G.player;
  if (!boxBlocked(p.x, p.y, p.w, p.h)) return;
  for (let r = 2; r <= 32; r += 2)
    for (const [dx, dy] of [[0, r], [0, -r], [r, 0], [-r, 0], [r, r], [-r, r], [r, -r], [-r, -r]])
      if (!boxBlocked(p.x + dx, p.y + dy, p.w, p.h)) { p.x += dx; p.y += dy; return; }
}
...
function enterCave(id) {
  ...
  Audio2.sfx.stairs();
  G.owReturn = { screen: { ...G.screen }, x: G.player.x, y: G.player.y + 6 };
  G.cave = { id, def, key: 'cave:' + G.screen.x + ',' + G.screen.y, textN: 0, buyCd: 0 };
...
G.onEnemyDead = function (en) {
  // kill combo — feedback + a small meta bonus at milestones
  G.combo++; G.comboT = 90;
  if (G.combo && G.combo % 5 === 0) addPoints(1);
  // random drop — BOUNTY tilts the odds toward better, more frequent loot
  let r = Math.random();
  if (G.hasMod('bounty')) r *= 0.6;
  let kind = null;
...
G.onBossDead = function (boss) {
  ...
  SAVE.stats.bosses++; addPoints(5);      // record + meta reward
  G.shake = 16; G.hitStop = 10;           // heavy impact
  ...
  if (G.env === 'overworld' && !G.secrets.has(G.screen.x + ',' + G.screen.y)) {
    ...
    for (let ty = 0; ty < 11; ty++) for (let tx = 0; tx < 16; tx++) {
      if (m.map[ty][tx] === 'X' && Math.hypot(tx * 16 + 8 - cx, ty * 16 + 8 - cy) < 30) {
        ...
        SAVE.stats.secrets++; addPoints(2);
        Audio2.sfx.secret();
        renderBase();
        return; // one hidden cave per screen — stop after opening it
      }
    }
  }
};
...
function updateCave() {
  ...
  if (G.cave.buyCd > 0) G.cave.buyCd--;

  for (const it of caveItems()) {
    if (!rectsHit({ x: it.x, y: it.y, w: 10, h: 12 }, p.hitbox())) continue;
    if (it.shop) {
      // cooldown + step-back keeps one approach = one purchase, even while the
      // player holds the stick toward the item (otherwise it re-buys every ~4 frames)
      if (G.cave.buyCd > 0) continue;
...
function drawHUD() {
  ...
  if (G.env === 'overworld') {
    // trail of every screen already walked this run
    ctx.fillStyle = '#007800';
    for (const k of G.visited) {
      const c = k.split(',');
      ctx.fillRect(16 + (+c[0]) * 8 + 1, 12 + (+c[1]) * 8 + 1, 6, 7);
    }
  ...
  for (let i = 0; i < p.hearts; i++) {
    ...
    ctx.fillStyle = full || half ? '#D82800' : '#787878';
  ...
  drawText(ctx, 'RUN ' + G.run, 18, 50, '#FCFCFC');
...
function render() {
  ...
  if (G.state === 'gen') { renderGen(); return; }
  if (G.state === 'records') { renderRecords(); return; }
  if (G.state === 'upgrades') { renderUpgrades(); return; }
  ...
  TextLayer.oy = HUD_H;   // room text sits below the HUD, like the canvas translate
  ctx.beginPath(); ctx.rect(0, 0, ROOM_W, ROOM_H); ctx.clip();
  if (G.shake > 0) { const m = Math.min(3, G.shake / 3); ctx.translate((Math.random() * 2 - 1) * m, (Math.random() * 2 - 1) * m); }
  ...
  // DARKNESS mutator: light only pools around the hero
  if (G.hasMod('dark') && G.player && (G.state === 'play' || G.state === 'scroll')) {
    const c = G.player.center();
    const g = ctx.createRadialGradient(c.x, c.y, 18, c.x, c.y, 94);
    g.addColorStop(0, 'rgba(0,0,0,0)'); g.addColorStop(1, 'rgba(0,0,0,0.86)');
    ctx.fillStyle = g; ctx.fillRect(0, 0, ROOM_W, ROOM_H);
  }
  if (G.combo >= 3 && G.comboT > 0) drawText(ctx, 'COMBO X' + G.combo, 96, 2, (G.t >> 1) % 2 ? '#FCB040' : '#FCFCFC');
...
// centered line helper (8px per glyph, matches drawText metrics)
function ctext(str, y, color) { drawText(ctx, str, 128 - str.length * 4, y, color); }
function seedStr() { return (WORLD_SEED >>> 0).toString(36).toUpperCase(); }
...
function renderTitle() {
  ...
  // triforce glow
  ctx.save();
  ctx.translate(112, 28);
  ctx.scale(4, 4);
  drawSprite(ctx, 'triforce', 0, 0);
  ctx.restore();
  ctext('ENDLESS', 72, '#FCB040');
  ctext('-THE LEGEND OF-', 86, '#FCFCFC');
  ctext('Z E L D A', 102, '#D82800');
  ctext('RUN ' + G.run + '   ' + SAVE.meta.points + ' PTS', 126, '#007800');
  const items = ['PLAY', 'UPGRADES', 'RECORDS'];
  for (let i = 0; i < items.length; i++) {
    const y = 152 + i * 16, sel = G.menu === i;
    ctext(items[i], y, sel ? '#FCFCFC' : '#787878');
...
function renderGen() {
  ...
  ctx.scale(4, 4);
  drawSprite(ctx, 'triforce', 0, 0, { remap: (G.t >> 3) % 2 ? { Y: 'W' } : null });
  ctx.restore();
  const dots = '.'.repeat(1 + ((G.genT >> 4) % 3));
  ctext('GENERATING YOUR', 116, '#FCFCFC');
  ctext('ZELDA' + dots, 132, '#FCB040');
  ctext('RUN ' + G.run, 160, '#00A800');
  ctext('SEED ' + seedStr(), 178, '#787878');
  if (G.mods.length) ctext('MODIFIERS: ' + G.mods.map(modName).join(' '), 196, '#D82800');
  if (G.genT > 80 && (G.t >> 4) % 2) ctext('PRESS ENTER', 218, '#787878');
}

function renderRecords() {
  ctx.fillStyle = '#000';
  ctx.fillRect(0, 0, 256, 240);
  ctext('-- RECORDS --', 22, '#FCB040');
  const s = SAVE.stats;
  const rows = [
    ['RUNS CLEARED', s.cleared], ['BEST TIME', s.bestTime ? fmtTime(s.bestTime) : '--'],
    ['FURTHEST RUN', s.furthest], ['BOSSES SLAIN', s.bosses],
    ['SECRETS FOUND', s.secrets], ['DEATHS', s.deaths], ['HERO POINTS', SAVE.meta.points],
  ];
  for (let i = 0; i < rows.length; i++) {
    const y = 52 + i * 20;
    drawText(ctx, rows[i][0], 40, y, '#FCFCFC');
    drawText(ctx, '' + rows[i][1], 196, y, '#FCB040');
  }
  if ((G.t >> 5) % 2) ctext('PRESS B TO GO BACK', 214, '#787878');
}

function renderUpgrades() {
  ctx.fillStyle = '#000';
  ctx.fillRect(0, 0, 256, 240);
  ctext('-- HERO SHOP --', 20, '#FCB040');
  ctext(SAVE.meta.points + ' POINTS', 36, '#00A800');
  for (let i = 0; i < UPGRADES.length; i++) {
    const u = UPGRADES[i], owned = !!SAVE.meta.unlocks[u.id], sel = G.menu === i, y = 66 + i * 22;
    if (sel) drawText(ctx, '>', 26, y, '#FCB040');
    drawText(ctx, u.name, 40, y, owned ? '#787878' : sel ? '#FCFCFC' : '#BCBCBC');
    drawText(ctx, owned ? 'OWNED' : (u.cost + ' PTS'), 188, y, owned ? '#787878' : '#FCB040');
  }
  if ((G.t >> 5) % 2) ctext('A BUY    B BACK', 220, '#787878');
}

function renderGameOver() {
  ctx.fillStyle = G.gameoverT < 40 ? '#A81000' : '#000';
  ctx.fillRect(0, 0, 256, 240);
  if (G.gameoverT > 40) {
...
function renderWin() {
  ...
  if (G.winT > 120) {
    const newRec = !SAVE.stats.bestTime || G.runT < SAVE.stats.bestTime;
    ctext((newRec ? 'NEW RECORD  ' : 'CLEARED IN  ') + fmtTime(G.runT), 172, newRec ? '#FCB040' : '#FCFCFC');
  }
  if (G.winT > 180) {
...
// ---------------- meta-progression upgrades (bought with points) ----------------
const UPGRADES = [
  { id: 'heart', name: '+1 START HEART', cost: 12 },
  { id: 'bombs', name: '+3 START BOMBS', cost: 8 },
  { id: 'bow',   name: 'START WITH BOW', cost: 20 },
  { id: 'ring',  name: 'BLUE RING',      cost: 30 },
  { id: 'tunic', name: 'BLUE TUNIC',     cost: 6 },
];
function applyUnlocks(p) {
  const u = SAVE.meta.unlocks;
  if (u.heart) { p.hearts = Math.min(8, p.hearts + 1); p.hp = p.maxHp; }
  if (u.bombs) { p.bombs = Math.min(8, p.bombs + 3); if (!p.bItem) p.bItem = 'bombs'; }
  if (u.bow)   { p.hasBow = true; if (!p.bItem) p.bItem = 'bow'; }
  if (u.ring)  { p.hasRing = true; }
  if (u.tunic) { p.tunic = true; }
}
function buyUpgrade(i) {
  const u = UPGRADES[i];
  if (!u || SAVE.meta.unlocks[u.id] || SAVE.meta.points < u.cost) { Audio2.sfx.shield(); return; }
  SAVE.meta.points -= u.cost; SAVE.meta.unlocks[u.id] = true; saveGame();
  Audio2.sfx.item();
}

// ---------------- menu navigation (works on keyboard + touch pad) ----------------
function menuNav(count) {
  if (G.menuCd > 0) G.menuCd--;
  if (!input.up && !input.down) G.menuCd = 0;   // release so a fresh tap always moves
  else if (G.menuCd === 0) {
    G.menu = (G.menu + (input.up ? count - 1 : 1)) % count;
    G.menuCd = 9; Audio2.sfx.text();
  }
}
function gotoTitle() { G.state = 'title'; G.menu = 0; G.menuCd = 6; }
...
function update() {
  ...
  // menus & full-screen states
  switch (G.state) {
    case 'title':
      if (input.up || input.down || input.left || input.right || input.a || input.b) Audio2.play('title');
      menuNav(3);
      if (input.a || input.start) {
        input.a = 0; input.start = 0;
        if (G.menu === 0) beginRun();
        else if (G.menu === 1) { G.state = 'upgrades'; G.menu = 0; G.menuCd = 6; }
        else { G.state = 'records'; }
      }
      return;
    case 'upgrades':
      menuNav(UPGRADES.length);
      if (input.a) { input.a = 0; buyUpgrade(G.menu); }
      if (input.b || input.sel || input.start) { input.b = input.sel = input.start = 0; gotoTitle(); }
      return;
    case 'records':
      if (input.a || input.b || input.sel || input.start) { input.a = input.b = input.sel = input.start = 0; gotoTitle(); }
      return;
    case 'gen':
      G.genT++;
      if ((G.genT > 80 && input.start) || G.genT > 170) { input.start = 0; startGame(); }
      return;
    case 'gameover':
      G.gameoverT++;
    ...
    case 'win':
      G.winT++;
      if (G.winT > 300 && input.start) { input.start = 0; nextRun(); }
      return;
  }

  // active gameplay (play / scroll / cave / itemGet)
  if (G.hitStop > 0) { G.hitStop--; if (G.state === 'play') return; }
  G.runT++;
  switch (G.state) {
...
// title -> roll mutators -> build a fresh random world -> "generating" screen
function beginRun() {
  Audio2.ensure();
  rollMutators();
  SAVE.run = G.run;
  if (G.run > SAVE.stats.furthest) SAVE.stats.furthest = G.run;
  saveGame();
  generateWorld(G.run, G.mods);
  G.state = 'gen';
  G.genT = 0;
}
...
function startGame() {
  ...
  // respawn falls back to the generated start until a dungeon is entered
  G.checkpoint = { screen: { x: START_SCREEN.x, y: START_SCREEN.y }, x: START_POS.x, y: START_POS.y };
  G.runT = 0; G.combo = 0; G.comboT = 0; G.shake = 0; G.hitStop = 0;
  G.visited.clear();               // fresh minimap trail per run
  G.state = 'play';
  loadOverworldScreen();
  Audio2.play('overworld');
  if (G.mods.length) showMsg(G.mods.map(modName).join(' + '));
}

// beat Ganon -> bank the clear, advance to the next run
function nextRun() {
  SAVE.stats.cleared++;
  if (SAVE.stats.bestTime === 0 || G.runT < SAVE.stats.bestTime) SAVE.stats.bestTime = G.runT;
  addPoints(10 + G.mods.length * 5);      // clear bonus + mutator risk bonus (saves)
  G.run++;
  SAVE.run = G.run;
  if (G.run > SAVE.stats.furthest) SAVE.stats.furthest = G.run;
  saveGame();
