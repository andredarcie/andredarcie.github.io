...
const SAVE_KEY = 'infinityZelda.save';
...
function renderGen() {
  ...
  ctext('RUN ' + G.run, 160, '#80D010');
  ctext('SEED ' + seedStr(), 178, '#585858');
...
function renderUpgrades() {
  ...
  ctext(SAVE.meta.points + ' POINTS', 36, '#80D010');
  for (let i = 0; i < UPGRADES.length; i++) {
    const u = UPGRADES[i], owned = !!SAVE.meta.unlocks[u.id], sel = G.menu === i, y = 66 + i * 22;
    if (sel) drawText(ctx, '>', 26, y, '#FCB040');
    drawText(ctx, u.name, 40, y, owned ? '#607860' : sel ? '#FCFCFC' : '#B0B0B0');
    drawText(ctx, owned ? 'OWNED' : (u.cost + ' PTS'), 188, y, owned ? '#607860' : '#FCB040');
  }
