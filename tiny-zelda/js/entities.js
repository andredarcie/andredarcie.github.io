...
class Player {
  ...
  draw(ctx) {
    ...
    const remap = this.hasRing ? { G: 'u' } : null;
...
class Explosion {
  constructor(x, y) { this.x = x; this.y = y; this.t = 24; this.r = 26; this.dead = false; this.hit = new Set(); }
  update() {
...
// ---------------- Pickups ----------------
class Pickup {
  // kind: heart | rupee | rupee5 | bomb | key | fairy
  constructor(kind, x, y) { this.kind = kind; this.x = x; this.y = y; this.w = 8; this.h = 8; this.t = 600; this.dead = false; }
  update() {
    if (this.kind === 'fairy') { this.x += Math.sin(this.t * 0.1) * 0.8; this.y += Math.cos(this.t * 0.13) * 0.6; }
  ...
  draw(ctx, t) {
    ...
    const s = { heart: 'heart', rupee: 'rupee', rupee5: 'rupee_y', bomb: 'bomb', key: 'key', fairy: 'fairy' }[this.kind];
...
class Enemy {
  constructor(type, x, y) {
    ...
    this.shootT = 90 + Math.random() * 150;
  ...
  update() {
    ...
    if (this.def.shoots && --this.shootT <= 0) {
      this.shootT = 140 + Math.random() * 160;
  ...
  walk() {
    ...
    const d = DIRS[this.dir];
    const nx = this.x + d[0] * this.def.speed, ny = this.y + d[1] * this.def.speed;
    if (!boxBlocked(nx, ny, this.w, this.h)) { this.x = nx; this.y = ny; }
    else this.moveT = 0;
  }
  ...
  fly() {
    if (--this.moveT <= 0) {
      ...
      this.fvx = Math.cos(a) * this.def.speed; this.fvy = Math.sin(a) * this.def.speed;
    }
    this.x = Math.max(2, Math.min(256 - this.w - 2, this.x + (this.fvx || 0)));
    this.y = Math.max(2, Math.min(176 - this.h - 2, this.y + (this.fvy || 0)));
  }
  ...
  hop() {
    ...
    this.vy += 0.12;
    this.y += this.vy;
    this.x = Math.max(2, Math.min(256 - this.w - 2, this.x + this.hx));
    ...
    if (this.kind === 'aquamentus') {
      ...
      if (this.t % 130 === 0) {
        ...
        for (const dy of [-0.35, 0, 0.35])
          G.projectiles.push(new Projectile('fireball', c.x, c.y, { x: -1, y: dy }, 1.6, 2, false));
      }
