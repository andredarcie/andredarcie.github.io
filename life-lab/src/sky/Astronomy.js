...
// rate), então a noite, que é quase só sono, passa depressa. Em tempo do céu um
...
export class Astronomy {
  ...
  // `time` é o tempo do céu, acumulado com rate — não o tempo da simulação.
  at(time) {
    ...
    const sun = Astronomy.#localDirection(declination, hourAngle);
    ...
    const moon = Astronomy.#localDirection(moonDeclination, hourAngle - phase * Math.PI * 2);
  ...
  static #localDirection(declination, hourAngle) {
